...
export default class RoleController extends Controller {
  ...
  public async list() {
    ...
    try {
      ...
      // 获取列表
      const allRole = await ctx.model.Role.findAll({
        attributes: [ 'id', 'name', 'describe', 'updated_at', 'parent_id', 'serial_num', 'menu_ids' ],
        where: whereParam,
  ...
  public async getRoleMap() {
    ...
    try {
      // 获取列表
      const allRole = await ctx.model.Role.findAll({
        attributes: [ 'id', 'name', 'describe', 'updated_at', 'updated_at', 'serial_num', 'menu_ids', 'parent_id' ],
        order: [[ 'updated_at', 'DESC' ]],
      });
      // 转换驼峰
      const roleList = allRole.map(item => ctx.helper.utils.lineToHumpObject(item));
  ...
  public async editPermission() {
    ...
    try {
      ...
      await ctx.model.Role.update({ menu_ids: ids }, {
        where: {
          id: roleId,
        },
      });
