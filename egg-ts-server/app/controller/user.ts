...
export default class UserController extends Controller {
  ...
  public async register() {
    ...
    try {
      const {
        ...
        email,
        code,
      } = ctx.request.body as { password: string, userName: string, email: string, code: string };
  ...
  public async addUser() {
    const ctx = this.ctx;
    try {
      const { password, userName, email, roleIds, avatar, nickName, profile } = ctx.request.body;
      const { bool, msg } = await ctx.service.user.checkUserNameAndEmail(userName, email);
      if (!bool) {
        await ctx.model.User.create({
          password, user_name: userName, email,
  ...
  public async editPassword() {
    ...
    try {
      ...
      if (code !== ctx.session.loginCode && emailCode !== ctx.session.editPasswordCode) {
        ...
        ctx.helper.response.handleSuccess({ ctx, data: true, msg: '修改密码成功' });
      }
    } catch (error) {
      console.log(error);
      ctx.helper.response.handleError({ ctx, data: false, msg: '修改密码失败' });
    }
  }
  ...
  public async login() {
    ...
    try {
      ...
      console.log(ctx.session!.loginCode, code);
      if (code !== loginCode) {
  ...
  public async getUserInfo() {
    ...
    try {
      ...
      const user = await ctx.model.User.findOne({
        attributes: [ 'user_name', 'role_ids', 'info', 'id' ],
        where: { [Op.and]: [{ id: uid }, { deleted: 0 }] },
      });
      ctx.helper.response.handleSuccess({
        ctx,
        msg: '查询用户信息成功',
  ...
  async editUser() {
    ...
    try {
      ...
      ctx.helper.response.handleSuccess({ ctx, data: true, msg: '更新用户信息成功' });
    } catch (e) {
      console.error(e);
      ctx.helper.response.handleError({ ctx, data: false, msg: '更新用户信息失败' });
    }
  }
  ...
  async getUserList() {
    ...
    try {
      ...
      const total = (await ctx.model.User.findAll()).length;
      ...
      const data = ctx.helper.utils.getPagination(
        list.map(item => {
          ...
          item.nickName = item.info.nickName;
          item.profile = item.info.profile;
  ...
  public async refreshToken() {
    ...
    try {
      ...
      const token = await ctx.service.user.generateToken(uid, scope);
      await ctx.service.user.saveToken(token, uid);
      ctx.helper.response.handleSuccess({ ctx, msg: '刷新token成功', data: token });
    } catch (e) {
      console.error(e);
      ctx.helper.response.handleError({ ctx, msg: '刷新token失败' });
    }
  }
}
